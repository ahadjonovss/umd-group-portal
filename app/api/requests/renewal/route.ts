...
export async function POST(req: NextRequest) {
  ...
  const discount = await getActiveDiscount(user.uid, categoryForRequest("subscription_renewal"), appId);
  const pct = discount?.percent ?? 0;
  const usd = Math.round(
    applyDiscount(renewalUsd({ serviceType, publishedPrice: app.publishedPrice }, pricing), pct)
  );
